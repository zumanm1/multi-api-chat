...
(async () => {
  ...
  try {
    ...
    const userMessage = await page.evaluate(() => {
      ...
      return userMsg ? userMsg.textContent : null;
    });
    console.log('User message sent to chat:', userMessage);
    
    // Check if we get a response or error message
    try {
      await page.waitForSelector('.message.assistant, .message.error', { timeout: 10000 });
      const responseType = await page.evaluate(() => {
        if (document.querySelector('.message.assistant')) return 'assistant';
        if (document.querySelector('.message.error')) return 'error';
        return null;
      });
      
      if (responseType === 'assistant') {
        ...
        const errorMessage = await page.evaluate(() => {
          return document.querySelector('.message.error').textContent;
        });
